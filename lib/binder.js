angular.module('Binder', [])
  .factory('binder', ['$timeout', '$q', 'unHash', 'syncEvents', 'binderTypes', 'modelWriter', 'deltaFactory', 'rtObserver',
    function ($timeout, $q, unHash, syncEvents, binderTypes, modelWriter, deltaFactory, rtObserver) {
      ...
      Binder.prototype.bindModel = function (type, scope, model) {
        switch(type) {
          case binderTypes.COLLECTION:
            rtObserver.observeCollection(type, scope, model, this.onModelChange);
            break;
        }
      };
