angular.module('OmniBinder', [])
  .factory('obBinder', ['$timeout', '$q', 'obSyncEvents', 'obBinderTypes', 'obModelWriter', 'obDelta', 'obObserver',
    function ($timeout, $q, obSyncEvents, obBinderTypes, obModelWriter, obDelta, obObserver) {
      ...
      Binder.prototype.bindModel = function (type, scope, model) {
        switch(type) {
          case obBinderTypes.COLLECTION:
            obObserver.observeCollection(type, scope, model, this.onModelChange.bind(this));
            break;
        }
      };
      ...
      Binder.prototype.push = function (newVal) {
        ...
        change = {
          ...
          name: String(obModelWriter.length(this))
        };
        delta = obDelta(change);

        obModelWriter.push(this, delta);
        ...
        delta.updateObject(obModelWriter.val(this));
      ...
      Binder.prototype.pop = function () {
        ...
        change = {
          ...
          object: obModelWriter.val(this),
          name: String(obModelWriter.length(this) - 1)
        };
        delta = obDelta(change);
