    ...
    this.check = function (binder, source, changes) {
      ...
      **/
      function checkUnsyncedChange (change, i) {
        //This is a splice change
        if (typeof change.addedCount === 'number') {
          angular.forEach(changes, function checkNewChange (newChange, j) {
            if (change.added === newChange.added &&
                change.index === newChange.index &&
                change.removed.length === newChange.removed.length) {
        ...
        //This is an object change
        else if (typeof change.name === 'string') {
          angular.forEach(changes, function checkNewChange (newChange, j) {
            if (change.name === newChange.name &&
