  ...
  .factory('$differ', function (syncEvents) {
    ...
    Differ.prototype.compareStrings = function (binder, delta, next) {
      ...
      if (delta.newVal.length > delta.oldVal.length) {
        Differ.prototype.findAddedString(binder, delta, next);
      }
      else if (delta.newVal.length < delta.oldVal.length) {
        Differ.prototype.findRemovedString(binder, delta, next);
      }
      else {
        Differ.prototype.findChangedString(binder, delta, next);
      }
    };

    Differ.prototype.compareArrays = function (binder, delta, next) {
      // There is a new array and no old array
      ...
      // Something has changed in an existing array.
        ...
        Differ.prototype.findAddedItem(binder, delta, next);
      }
      else if (delta.newVal.length < delta.oldVal.length) {
        // Something has been removed from the array.
        Differ.prototype.findRemovedItem(binder, delta, next);
      }
      else {
        // An object changed within the array.
        Differ.prototype.findUpdatedItem(binder, delta, next);
      }
    };

    Differ.prototype.checkArrayForDupe = function (binder, delta, next) {
      var found;
    ...
    Differ.prototype.determineDelta = function (binder, delta, next) {
      ...
      if (!delta.oldVal && delta.newVal) {
        ...
        Differ.prototype.compareArrays(binder, delta, next);
      }
      else if (typeof delta.newVal === 'string' && typeof delta.oldVal === 'string') {
        Differ.prototype.compareStrings(binder, delta, next);
      }
      else {
