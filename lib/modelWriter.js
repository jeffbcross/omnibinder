angular.module('OmniBinder')
  .factory('obModelWriter', ['obBinderTypes', 'obSyncEvents', function (obBinderTypes, obSyncEvents) {
    ...
    ModelWriter.prototype.length = function (binder) {
    ...
    ModelWriter.prototype.processChanges = function (binder, delta) {

      angular.forEach(delta.changes, function (change) {
        switch (change.type) {
          case obSyncEvents.NEW:
            this.newFromProtocol(binder, change);
            break;
          case obSyncEvents.DELETED:
            this.deletedFromProtocol(binder, change);
            break;
          case obSyncEvents.UPDATED:
    ...
    ModelWriter.prototype.val = function (binder) {
      return angular.copy(binder.scope[binder.model]);
    };

    var obModelWriter = Object.create(ModelWriter.prototype);
    ModelWriter.call(obModelWriter);
    return obModelWriter;
  }]);
