...
angular.module('SyncDeployd', [])
  .factory('syncDeployd', function ($q, $rootScope, dpd) {
    ...
    DeploydTransport.prototype.update = function (query, obj) {
    ...
    DeploydTransport.prototype.subscribe = function (query, callback) {
      ...
      dpd[query.path].on('create' + single, function (data) {
        callback && callback.call(this, 'create', data);
      });

      dpd[query.path].on('delete' + single, function (data) {
        callback && callback.call(this, 'delete', data);
      });

      dpd[query.path].on('update' + single, function (data) {
        callback && callback.call(this, 'update', data);
      });
    };
