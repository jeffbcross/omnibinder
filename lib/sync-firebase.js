...
angular.module('SyncResource')
  .factory('syncFirebase', function ($q, $rootScope, syncEvents) {
    ...
    FirebaseProtocol.prototype.change = function (query, delta) {
      console.log('change in Firebase', query, delta);
    };

    FirebaseProtocol.prototype.create = function (query, delta) {
      console.log('creating', query, delta);
      var deferred = $q.defer();

      this.fb.child(query.path).push(delta.data, function (err) {
    ...
    FirebaseProtocol.prototype.update = function (binder, query, delta) {
      if (!query || !query.path) throw new Error('Update requires query.path');
      var deferred = $q.defer();
      
      this.fb.child(query.path).update(delta.data, function (err) {
        if (err) return deferred.reject(err);
        deferred.resolve();
      });
    ...
    FirebaseProtocol.prototype.subscribe = function (binder, callback) {
      ...
      // this.fb.child(binder.query.path).once('value', function (snapshot) {
