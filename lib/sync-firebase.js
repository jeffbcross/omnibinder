...
angular.module('SyncResource')
  .factory('syncFirebase', function ($q, $rootScope, $modelWriter, syncEvents) {
    ...
    FirebaseProtocol.prototype.create = function (binder, delta) {
      ...
      var ref = this.fb.child(binder.query.path).push(delta.data, function (err) {
        if (err) return deferred.reject(err);
        if (typeof delta.data === 'object' && typeof ref !== 'undefined' && typeof delta.position === 'number') {
          // $modelWriter.updatedFromProtocol(binder, delta); //Method is not robust enough
          binder.scope[binder.model][delta.position].id = ref.name(); //TODO: This is hacky
