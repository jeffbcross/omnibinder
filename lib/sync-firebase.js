...
angular.module('SyncResource')
  .factory('syncFirebase', function ($q, $rootScope, $modelWriter, syncEvents) {
    ...
    FirebaseProtocol.prototype.create = function (binder, delta) {
      ...
      delete delta.data.$$hashKey; // TODO: HACK
      
      var ref = this.fb.child(binder.query.path).push();
      delta.data.id = ref.name();
      
      ref.setWithPriority(delta.data, delta.position, function (err) {
        if (err) return deferred.reject(err);
        deferred.resolve(id? id : null);
      });
