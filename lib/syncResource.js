...
angular.module('SyncResource', [])
  .factory('$syncResource', function ($rootScope, $q) {
    ...
    Synchronizer.prototype.addFromProtocol = function (model, message) {
    ...
    Synchronizer.prototype.onProtocolChange = function (ev, message, model, deferred) {
      ...
      switch(ev) {
        ...
        case this.events.ADD:
          ...
          this.addFromProtocol(this.config.scope[model], message);
          break;

        case this.events.REMOVE:
          if (Array.isArray(this.config.scope[model])) {
            angular.forEach(this.config.scope[model], function (item, i) {
              if (found) return;
              if (angular.equals(item, message)) {
                self.config.scope[model].splice(i, 1);
                found = true;
              };
            });
          }
