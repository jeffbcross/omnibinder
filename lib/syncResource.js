  ...
  .factory('$syncResource', function ($rootScope) {
    function Synchronizer (config) {
      ...
      var bindToObject = function () {
        ...
        protocol.getObject(config.id, function (obj) {
          scope[config.model] = obj;
          scope.$digest();
        });
        
        //Listen for changes
        protocol.subscribeToObject(config.id, function (message) {
        ...
        scope.$watch(config.model, function (newVal, oldVal) {
          if (!angular.equals(newVal, oldVal) && !angular.equals(newVal, lastUpdate)) {
            protocol.updateObject(config.id, newVal);
          }
      ...
      var bindToCollection = function () {
        protocol.getCollection(function (models) {
          scope[config.model] = models;
          scope.$digest();
        });
        
        protocol.subscribeToCollection(function (event, model) {
          switch (event) {
            case 'create':
