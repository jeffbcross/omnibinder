    ...
    Synchronizer.prototype.sendToProtocol = function (binder, delta) {
      switch (delta.type) {
        case syncEvents.UPDATE:
          this.config.protocol.update(binder.query, delta);
          break;

        case syncEvents.REMOVE:
          this.lastDelta = angular.copy(delta);
          this.config.protocol.remove(binder.query, delta);
          break;

        case syncEvents.ADD:
          this.lastDelta = angular.copy(delta);
          this.config.protocol.create(binder.query, delta);
          break;
        default:
          this.config.protocol.change(binder.query, delta);
      }
    };
    ...
    */
    Synchronizer.prototype.onModelChange = function (newVal, oldVal, binder) {
      ...
      if (typeof binder.onModelChange === 'function') {
        binder.onModelChange(binder, delta, function () {
          self.sendToProtocol(binder, delta);
        });
      }
      else {
        this.sendToProtocol(binder, delta);
      }
    };
    ...
    Synchronizer.prototype.onProtocolChange = function (binder, delta) {
      ...
      if (typeof binder.onProtocolChange === 'function') {
        binder.onProtocolChange(delta, function () {
